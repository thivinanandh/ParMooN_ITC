...
//   OutPut("NBF b " << NBF << endl);
  ...
  for (i=0;i<N_RootCells;i++)
  {
    ...
    for (j=0;j<N_Faces;j++)
    {
      if (!NewJoints[auxi++])
      {
        ...
        if (aux1 == 2)
        {
          ...
          NewJoints[aux1*6 + j] = NewJoints[aux2*6 + k] = new TJointEqN(CellTree[aux2], CellTree[aux1]);
    ...
    for (j=0;j<N_Faces;j++)
    {
      ...
      //if(NewJoints[auxi]->GetType() == InterfaceJoint3D)
       //  cout << auxi << " cell: " << i << " joint: " << j << " auxi " <<  auxi << endl;
      auxi++;
    }
  }
  ...
  // cout<< " end N_RootCells : " << N_RootCells << endl;
  for(i=0;i<N_RootCells;i++)
  {
...
/** Implemented by Sashi 08 Oct 2019 */
int TDomain::TetGen(char *SMESH)
{
 //======================================================================
 ...
 //======================================================================
  ...
  double *Vertices;
  int N, N_Cells, CurrVertex, N_Vertices, ID, N_Faces, N_G, RefLevel=0;
...
//  TBdSphere *UpdateParam;
 ...
 std::ifstream dat(SMESH);
  ...
  dat.getline (line, 99);
  dat >> N_Faces >> N_FVert; // determine the number of vertices and alocate memory
 ...
 if(N_FVert==3)
  for(i=0;i<N_Faces; i++)
   ...
   {
    cout<< "Number of face vertices should be 3 or 4, see TetGen() in ReadGeo !!!!!!!!! "<< endl;
    exit(0); 
   }

  dat.close();

    BoundPart = this->GetBdPart(0);
    N = BoundPart->GetN_BdComps();

    /** use PRM to set the BdParam instead of below code*/
...
//     search neighbours
     ...
     if (Out.trifacemarkerlist[i]) // 0 for inner edges and Boundcomp+1 for Boundedge respect
      {
       ...
       bdcomp = this->GetBdPart(0)->GetBdComp(CurrComp);
       ...
       if (CurrNeib == 2)
        {
         if(bdcomp->IsFreeBoundary())
           Joint = new TIsoInterfaceJoint3D(bdcomp, T, S, CellTree[Neib[0]], CellTree[Neib[1]] );
          else
           Joint = new TInterfaceJoint3D(bdcomp, T, S, CellTree[Neib[0]], CellTree[Neib[1]] );
        }
       else
        {
         if(bdcomp->IsFreeBoundary())
  ...
  if (Joint->GetType() == InterfaceJoint3D || Joint->GetType() == IsoInterfaceJoint3D)
      ...
      else if (Joint->GetType() == JointEqN)
         { Joint->SetMapType(); }
...
/** Implemented by Sashi 08 Oct 2019 */
TDomain *TDomain::GetSurfMesh(int *Cell_IDX, int *Joint_IDX)
{
...
//   TBoundComp *BoundComp;
  ...
  for(i=0;i<N_Cells;i++)
  {
    ...
    for(l=0;l<k;l++)
     {
       joint = Me->GetJoint(l);
       if(joint->GetType() == BoundaryFace || joint->GetType() == IsoBoundFace || joint->GetType() == IsoInterfaceJoint3D )
         N_SurfCells++;
     } // endfor l
  } // endfor i
...
//    cout << " N_vertices:  " << N_BoundVert << endl;
  ...
  // for(i=0;i<N_SurfCells;i++)
  //  cout << "ReadGeo: Cell_No:  " <<Cell_IDX[i] << " Joint_No  " << Joint_IDX[i] << endl;
