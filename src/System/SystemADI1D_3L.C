...
TSystemADI1D_3L::TSystemADI1D_3L(TSystemADI1D **systemADI, int *l_NVertices, double *l_StartEnd, BoundCond1D **boundConLminLMax, 
                    DoubleFunctND **growthAndB_Nuc, TFEFunction2D *spatialFeFunction, double **XPos):TSystemADI1D()
{        
 ...
 for(int i=0; i<3;++i)
  SystemADI[i] = new TSystemADI1D(l_NVertices[i], l_StartEnd[2*i], l_StartEnd[2*i+1], boundConLminLMax[i],
                                  growthAndB_Nuc[i], XPos[i]);                               
}

int GetAgeIndex(int N_AgeGroup, double *MaxAgeOfGroup, double val)
{
if (TDatabase::ParamDB->PBE_P10<0.)
 {
  if(val<MaxAgeOfGroup[0])
   {
    return 0; 
   }

  for(int i=1; i<N_AgeGroup; i++)
   {
     if(val>=MaxAgeOfGroup[i-1] && val<MaxAgeOfGroup[i])
     {
      return i;
     }
   }
 ...
 {
  return int(TDatabase::ParamDB->PBE_P10);
 }
...
// int_Lv_La at l_d=0
void TSystemADI1D_3L::Int_B_Nuc(double *IntValue, double **B_NucValue, DoubleFunctND *GrowthAndB_Nuc, 
                             double **SuscepPopRatio, int N_AgeGroup, double *MaxAgeOfGroup)
{
  ...
  int i, AgeIndex, i1, i2, l1, l2, j, N_L1Cells = L1Coll->GetN_Cells();
  ...
  int N_QuadPts;
  double *Weights, *zeta;
  double Mult0, interval_length;
  qf1->GetFormulaData(N_QuadPts, Weights, zeta);
  TRefTrans1D *F_K2, *F_K1;
  double mult1, mult2, Coords[8], IVal[3];
  // double *vol_Xpos=new double[N_Xpos], *vol_Xpos1=new double[N_Xpos*N_QuadPts], vol=0., vol1=0.;
  // double x0,x1,y0,y1;
  ...
  // double norm = TDatabase::ParamDB->FS_U; // multiplied in the GrowthAndB_Nuc function in example file!

  //memset(vol_Xpos, 0, N_Xpos*SizeOfDouble);
    ...
    for(i1=0; i1<N_L1Cells; ++i1) //lv
     {
      ...
      // cout << "N_QuadPts " << N_QuadPts <<endl;

      // memset(vol_Xpos1, 0, N_Xpos*N_QuadPts*SizeOfDouble);      

      for(j=0; j<N_AgeGroup;++j)
       memset(IntValue_XposL0quadL1quad_temp[j], 0, N_Xpos*N_QuadPts*SizeOfDouble); //N_QuadPtsL2
      ...
      for(l2=0; l2<N_QuadPts; l2++)    
       {
        ...
        AgeIndex = GetAgeIndex(N_AgeGroup, MaxAgeOfGroup, X2[l2]);

        for(j=0; j<N_AgeGroup;++j)
         IntValue_Xpos[j] = IntValue_XposL0quadL1quad_temp[j] + l2*N_Xpos;
        ...
        for(l1=0; l1<N_QuadPts; ++l1)
         {
           ...
           for(i=0; i<N_Xpos; i++)
           {
            ...
            Coords[5] = SuscepPopRatio[AgeIndex][i];
            Coords[6] = SuscepPopRatio[N_AgeGroup+AgeIndex][i];  
            Coords[7] = SuscepPopRatio[2*N_AgeGroup+AgeIndex][i];     

            GrowthAndB_Nuc(8, Coords, IVal);

            // IntValue_Xpos[AgeIndex][i] += IVal[1]*mult1;
            IntValue_Xpos[AgeIndex][i] += IVal[1]*mult1;
            
            // if(AgeIndex==4)
              // vol_Xpos1[l2*N_Xpos + i] += IVal[1]*mult1/interval_length;
      
           }// i=0; i<N_Xpos  
         } //  for(l1=0; l1 // l_v, N_QuadPts

        
         for(i=0; i<N_Xpos; i++)
         {
          B_NucValue[AgeIndex][i] += IntValue_Xpos[AgeIndex][i]*mult2;
          // vol_Xpos[i] += vol_Xpos1[i]*mult2;
         }// i
         // vol1 +=vol*mult2;   
       } // l2 : la, N_QuadPts
     } //i1 : lv, N_L1Cells
...
//   { 
//    for(i=0; i<N_Xpos; i++)
//      cout << "j " << j << " B_nuc " <<B_NucValue[j][i] << " vol "<< endl;
...
//      cout << "j " << j << " B_nuc " <<sum  << endl;
//     //  cout << "j " << j << " B_nuc " <<sum << " vol "<< vol_Xpos[i] << endl;
...
void TSystemADI1D_3L::Init(int n_Xpos, double *xpos, int *n_LDof, int *n_LnodalPos, double **lnodalPos, 
                           double *sol_XdoLdof, int N_AgeGroup)
{
...
//  if(SystemADI[2]->IsdGdisc())
 ...
 for(int i=0; i<3;++i)
  ...
  IntValue_XposL0quadL1quad_temp = new double*[N_AgeGroup];
  for(int i=0; i<N_AgeGroup;++i)
  IntValue_XposL0quadL1quad_temp[i] = new double[N_Xpos*MaxN_QuadPoints_1D*MaxN_QuadPoints_1D];  
  
}


void TSystemADI1D_3L::Solve(int ADI_SolveIdx, CoeffFctND *Bilinear, double *IntL_Values, 
                      double **SuscepPopRatio, int N_AgeGroup, double *MaxAgeOfGroup)
{
  double *Sol_ALL, *Sol_L, *Sol_LL, *Sol_LLL;
  int i, j, k, l, m, n, AgeIndex;
  double *LLLpos, *LLpos, *Lpos;
  double Coords[8], interval_length;
    ...
    for(j=0; j<l; ++j)
     {
      ...
      // note  LLLpos is age only for 
      AgeIndex = GetAgeIndex(N_AgeGroup, MaxAgeOfGroup, LLLpos[j]);
      Coords[5] = SuscepPopRatio[AgeIndex][i];
      Coords[6] = SuscepPopRatio[N_AgeGroup+AgeIndex][i];
      Coords[7] = SuscepPopRatio[2*N_AgeGroup+AgeIndex][i];      
      // if(LLLpos[j]<19./125.) 
      //   { Coords[5] = SuscepPopRatio[1][i]; }   // age-base SuscepPopRatio  
      // else if (LLLpos[j]<45./125.)             
      //   { Coords[5] = SuscepPopRatio[2][i]; }  
      // else
      //   { Coords[5] = SuscepPopRatio[0][i]; }
      ...
      for(k=0; k<m; ++k)
       { 
        ...
        SystemADI[ADI_SolveIdx]->Solve(8, Coords, Bilinear, Sol_L);
       }
     }
  }
