...
TSystemADI1D::TSystemADI1D(int N_L, double start, double end, BoundCond1D *boundConLminLMax, 
                           DoubleFunctND *growthAndNuc, double *XPos): TSystemADI(growthAndNuc)
{
 //mesh
 Domain_Intl = new TDomain();
 this->Generate1DMesh(start, end, N_L, XPos);
...
void TSystemADI1D::Generate1DMesh(double Start, double End, int N_Cells, double *XPos)
{
  int i, j, N_Vert;
  int *Lines;
  double len, h, x, y, *X;
