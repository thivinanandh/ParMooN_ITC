...
TADISystem1D::TADISystem1D(TFEFunction1D *FEFunction_intl, TSquareMatrix1D *M_internal, TSquareMatrix1D *A_internal,
                           ...
                           DoubleFunctND *initial, DoubleFunct2D *bDValue_LMin, DoubleFunct2D *bDVal_LMax,
                           double *Sol,  double *OldSol, double *b, 
                           double *Defect, double *Intlposl, int n_coord, double *xpos, DoubleFunctND *growthAndNuc): TADISystem(Sol, OldSol, b, Defect, growthAndNuc)
{
//  int i;
  ...
  N_Coord = n_coord;
  X = new double[N_Coord]; // +1 for pos of this ordinate 
...
// 3D
TADISystem1D::TADISystem1D(TFEFunction1D *FEFunction_intl, TSquareMatrix1D *M_internal, TSquareMatrix1D *A_internal,
                           int n_coord, double *xpos, double *Sol,  double *OldSol, double *b,
...
// sol will be returned in SolFromX
...
// sol will be returned in SolFromX
void TADISystem1D::SolveAllLevels(double *MatValues_orig, double *SolFromX,  double *AggrRhs, CoeffFctND *BilinearCoeffs,
                                  double *GrowthParameters, double tau, BoundCond cond_Lmin, BoundCond cond_Lmax)
{
 int i, j, k, l;
 double gamma=0., BDValue1, BDValue2;
 double G, Bnuc,  Out[2], *MatValues, limit=1.e-8;
 double G_DimLessFact = TDatabase::ParamDB->REACTOR_P27;
 ...
 //     // PSD at L_{max}
  ...
  /** set I as non-negative */
   for(j=0;j<N_V;j++)
    ...
    if(GetGrowthAndNuc)
    {
      // Inn[0] = Z; // l2 internal coordinate value
      // Inn[1] = T;

      GetGrowthAndNuc(4, GrowthParameters, Out);
