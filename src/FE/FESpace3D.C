...
/** for rte */
void TFESpace3D::GetSurfDOFPosition(int N_Cells_Intl, int *SurfCells, int *SurfJoints, double *x)
{
  int ii, i,j,k, N_JointDOF, disp;
  TBaseCell *cell;
  TJoint *joint;
  JointType jointtype;
  FE3D FEid;
  ...
  double X[MaxN_BaseFunctions3D], Y[MaxN_BaseFunctions3D];
  double Z[MaxN_BaseFunctions3D], absdetjk[MaxN_BaseFunctions3D];
  values = new double[MaxN_BaseFunctions3D];

  RefTransArray = TFEDatabase3D::GetRefTrans3D_IDFromFE3D();
  disp = 0;
  
  for(ii=0;ii<N_Cells_Intl;ii++)
  {
    i = SurfCells[ii];
    j = SurfJoints[ii];
    DOF = GlobalNumbers + BeginIndex[i];
    ...
    nf = TFEDatabase3D::GetNodalFunctional3DFromFE3D(FEid);
    nf->GetPointsForFace(j, N_Points, xi, eta, zeta);
    if(N_Points==0)
     {
      cout << "fespace3d.C, No DOF on the face! " << N_Points <<endl;
      exit(0);
     }
    ...
    nf->GetFaceFunctionals(Collection, cell, j, X, values);
    nf->GetFaceFunctionals(Collection, cell, j, Y, values+N_JointDOF);
    nf->GetFaceFunctionals(Collection, cell, j, Z, values+(2*N_JointDOF));                            
 
    //valuex = x+(ii*N_JointDOF);

    for(j=0;j<N_JointDOF;j++)
      {
       x[disp] = values[j]; // x_j
       disp++;
       x[disp] = values[j+N_JointDOF]; // y_j
       disp++;       
       x[disp] = values[j+(2*N_JointDOF)]; //z_j
       disp++;
      }
  } // endfor i
