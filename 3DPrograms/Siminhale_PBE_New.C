...
int main(int argc, char *argv[])
{
  ...
  while (TDatabase::TimeDB->CURRENTTIME < end_time)
  {
    ...
    for (l = 0; l < N_SubSteps; l++) // sub steps of fractional step theta
    {
      ...
      // Solve the internal System for every physical x position
      for (int i = 0; i < N_PhySpacePts; i++)
      {
        // Solve the internal system for the current x position
        int N_Param = 1;
        double Coords[8]; // To pass additional values and co-rdinates
        double* current_internal_solution = solution_all + i * N_InternalPts;
        Coords[0] = Xpos_internal[0][i];
        Coords[1] = Xpos_internal[0][i];
        
        // copy the solution to the old solution
        memcpy(solution_internal, current_internal_solution, N_InternalPts * SizeOfDouble);

        // Assemble the internal system matrix
        Internal_System->AssembleMassMat();

        // Assemble the internal system matrix
        Internal_System->Solve(N_Param, Coords, BilinearCoeffs, current_internal_solution);
      ...
      for (int i = 0; i < N_InternalPts; i++)
      {
        ...
        int N_Param = 1;
        double Coords[8]; // To pass additional values and co-rdinates
        double* current_internal_solution = solution_all + i * N_PhySpacePts;
        Coords[0] = Xpos_internal[0][i];
        Coords[1] = Xpos_internal[0][i];
