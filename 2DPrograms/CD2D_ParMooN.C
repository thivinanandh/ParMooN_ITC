...
int main(int argc, char* argv[])
{
  ...
  TFEVectFunct2D *Grad_FeFunction;  
  double values[3], *GradSol = new double[2*N_DOF], *GradPos = new double[2*N_DOF];
  memset(GradSol, 0, 2*N_DOF*SizeOfDouble);
  memset(GradPos, 0, 2*N_DOF*SizeOfDouble);
  Grad_FeFunction = new TFEVectFunct2D(Scalar_FeSpace, (char*)"Grad_", (char*)"Grad_", GradSol, N_DOF, 2);   
  Grad_FeFunction->GridToData();

  for(i=0; i<N_DOF; i++)
    {
     Scalar_FeFunction->FindGradient(GradSol[i] , GradSol[N_DOF+i], values);  
     GradPos[i] = values[1];
     GradPos[N_DOF+i] = values[2];     
    }

   memcpy(GradSol, GradPos, 2*N_DOF*SizeOfDouble);
  //======================================================================
  // produce outout
  //======================================================================
  ...
  Output->AddFEVectFunct(Grad_FeFunction);
