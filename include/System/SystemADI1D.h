...
class TSystemADI1D : public TSystemADI
{
  ...
  private:
   ...
   void Generate1DMesh(double Start, double End, int N_Cells, double *XPos);

  public:
    /** constructor */
    TSystemADI1D();

    TSystemADI1D(int N_L, double start, double end, BoundCond1D *boundConLminLMax, DoubleFunctND *growthAndNuc, double *XPos);
